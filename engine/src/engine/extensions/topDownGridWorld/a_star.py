...
def is_obstacle(grid, x, y):
    if hasattr(grid.tiles[x][y], 'is_occupied'):

        return not grid.tiles[x][y].is_walkable or grid.tiles[x][y].is_occupied
    return not grid.tiles[x][y].is_walkable

def is_obstacle_ghost(grid, x, y):
    return not grid.tiles[x][y].is_walkable
...
def get_neighbors_ghost(grid, node, include_diagonal=True):
    candidates = [
        Node(node.x, node.y - 1),
        Node(node.x, node.y + 1),
        Node(node.x - 1, node.y),
    ...
    return [n for n in candidates if is_valid(n, grid) and not is_obstacle_ghost(grid, n.x, n.y)]
...
def a_star_ghost(grid, start, goal):
    open_set = _PriorityQueue()
    came_from = {}
    g_score = {start: 0}
    f_score = {start: euclidean_distance(start, goal)}
    ...
    while open_set.queue:
        ...
        for neighbor in get_neighbors_ghost(grid, current):
            tentative_g = g_score[current] + euclidean_distance(current, neighbor)
...
def find_path_ghost(grid, start, goal):
    return a_star_ghost(grid, start, goal)
