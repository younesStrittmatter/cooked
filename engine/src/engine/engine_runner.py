...
class EngineRunner:
    ...
    def start(self):
        ...
        def start_engine_loop_with_max_ticks():
            max_ticks = None
            if self.session and self.session.max_game_time:
                max_ticks = self.session.max_game_time * self.tick_rate
            self.engine.start_loop(max_ticks=max_ticks)
        self.engine_task = self.session.socketio.start_background_task(start_engine_loop_with_max_ticks)
        self.engine.agent_map = self.agent_map
    ...
    def _run_agents(self):
        ...
        while not self.game.done:
            ...
            if (sim_time - last_action_sim_t) + 1e-12 >= decision_dt:
                ...
                for agent_id, controller in self.agent_map.items():
                    if getattr(controller, "sync_on_tick", False):
                        continue
